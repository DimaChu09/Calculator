...
public class Main {
    ...
    public int romanToInteger(String str) {
        int result = 0, i = 0;
        while (i < str.length()) {
            if (i + 2 < str.length() && romanMap.containsKey(str.substring(i, i + 3))) {
                result += romanMap.get(str.substring(i, i + 3));
                i += 3;
            } else if (i + 1 < str.length() && romanMap.containsKey(str.substring(i, i + 2))) {
    ...
    public static void main(String[] args) {
        ...
        if (isRomanA != isRomanB) {
            throw new IllegalArgumentException("Нельзя смешивать римские и арабские цифры");
        }
        ...
        if (a < 1 || a > 100 || b < 1 || b > 100) {
            throw new IllegalArgumentException("Числа должны быть от 1 до 100 включительно");
        }
